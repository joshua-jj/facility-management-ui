        ...
        <div className="">
            ...
            <div className="grid grid-cols-7 gap-2">
                {['Sun', 'Mon', 'Tue', 'Wed', 'Thu', 'Fri', 'Sat'].map(day => (
                    <div key={day} className="text-center text-[0.65rem] uppercase font-medium">{day}</div>
                ))}
            ...
            <FullscreenModal open={eventModal} onClickAway={() => setEventModal((prev) => !prev)}>
                {selectedDate && (
                    <div className="p-8 mt-4 text-[#0F2552] bg-gray-100 rounded-md">
                        <h2 className="font-bold text-lg">Events on {dayjs(selectedDate).format('MMMM D, YYYY')}</h2>
