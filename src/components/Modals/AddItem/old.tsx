...
const AddItem: React.FC<AddItemModalProps> = ({ className, children }) => {
  const dispatch = useDispatch();
  const { IsCreatingItem } = useSelector((s: RootState) => s.item);
  const { allDepartmentsList } = useSelector((s: RootState) => s.department);
  const { allStoresList } = useSelector((s: RootState) => s.store);
  ...
  // const formRef = useRef<FormsyRef | null>(null);
  ...
  const handleSubmit = (data: ItemForm) => {
    // include the last filled form if not already saved
    if (data.name) {
    ...
    // Submit all collected items
    items.forEach((item) => {
      dispatch(itemActions.createItem(item) as unknown as UnknownAction);
    });
  ...
  const addMoreItem = () => {
    if (formRef.current) {
      ...
      formData.departmentId = Number(department?.id);
            ...
            <div className="mb-4 space-y-2 border-b border-gray-200 pb-4">
              {items.map((item, idx) => (
                <div key={idx}>
                  ...
                  <div className="w-full px-4 py-2 bg-[#0F255233] border border-[#0F255226] rounded text-[#0F2552] font-medium">
                    {item.name}
                  </div>
                </div>
              ))}
