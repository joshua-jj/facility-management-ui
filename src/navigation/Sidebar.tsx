...
const Sidebar = () => {
  ...
  return (
    <div className="bg-white px-[1.8rem] py-[0.93rem] text-[#0F2552] border-[0.5px] border-r-[#E1E3E7] shadow-[0px_16px_32px_0px_rgba(189,189,189,0.25)]">
      <Link href="/" passHref className="">
        <div className="flex items-center">
          <EGFMLogoIcon className="" />
          <span className="ml-2 text-[#32323d] text-[16px] font-bold leading-[21px] text-left">
            Logistics
          </span>
        </div>
      </Link>
        ...
        {filteredRoutes?.map((pageRoute, index) => (
          ...
          >
            <Link href={pageRoute?.link} className="flex items-center gap-x-4">
              ...
              <span className="font-medium text-`nowrap33`">{pageRoute?.label}</span>
            </Link>
          </li>
        ))}
      </ul>
