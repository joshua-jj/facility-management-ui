...
const Sidebar = ({ mobileOpen = false, onMobileClose }: SidebarProps) => {
   ...
   const activeRoute = useCallback(
      (link: string) => router?.pathname === link,
      [router?.pathname],
   );
   ...
   // ── Shared route list ───────────────────────────────────────────────────
   const renderRoutes = (isCollapsed: boolean) =>
      filteredRoutes?.map((pageRoute, index) => {
         ...
         const showSectionHeader = !!pageRoute.section && pageRoute.section !== prevSection;
         return (
            <React.Fragment key={pageRoute.id}>
               ...
               {showSectionHeader && isCollapsed && (
                  <motion.li
                     variants={itemVariants}
                     className={classNames('mt-3 mb-1 mx-auto h-[1px] w-6', isDark ? 'bg-white/15' : 'bg-[#0F2552]/15')}
                  />
               )}
               <motion.li variants={itemVariants} className="mb-1">
                  <Link
                     ...
                     onFocus={(e) => showTooltip(e as unknown as React.MouseEvent<HTMLElement>, pageRoute?.label)}
                     onBlur={hideTooltip}
                     className={classNames(
                  ...
                  >
                     ...
                     <div className={classNames('shrink-0 transition-transform duration-200', isActive ? 'scale-100' : 'scale-[0.9] group-hover:scale-110')}>
                        {pageRoute?.icon}
                     </div>
                     {!isCollapsed && (
                     ...
                     {!isCollapsed && !isActive && (
                        <span className={classNames('ml-auto opacity-0 group-hover:opacity-100 transition-opacity duration-200', isDark ? 'text-white/40' : 'text-[#0F2552]/30')}>
                           <ChevronRight />
                        </span>
                     )}
                  </Link>
   ...
   const renderProfile = (isCollapsed: boolean) =>
      fullName ? (
         <motion.div
            ...
            initial="hidden"
            animate="visible"
            className={classNames('px-3 py-3', isDark ? 'border-t border-white/10' : 'border-t border-[var(--border-default,#e5e7eb)]')}
         >
            {isCollapsed ? (
               ...
               <div className="flex items-center gap-x-3 min-w-0">
                  ...
                  <div className="flex flex-col min-w-0">
                     <span className={classNames('text-sm font-semibold truncate leading-tight', isDark ? 'text-white' : 'text-[var(--text-primary,#0F2552)]')}>
                        {fullName}
                     </span>
                     {roleLabel && (
                        <span className={classNames('text-xs truncate leading-tight mt-0.5', isDark ? 'text-white/40' : 'text-[var(--text-hint,#6b7280)]')}>
                           {roleLabel}
                        </span>
                     )}
                  </div>
   ...
   return (
      <>
         {/* ── DESKTOP sidebar — flex child, collapse/expand works normally ── */}
         <div
            className={classNames(
               'animate-sidebar-enter hidden md:flex flex-col h-full overflow-hidden transition-[width] duration-300 ease-in-out',
               isDark ? 'sidebar-dark' : '',
            )}
            style={{ width: collapsed ? 72 : 260, minWidth: collapsed ? 72 : 260, ...sharedStyles }}
         >
            {/* Logo */}
            <div className="px-4 py-5">
               <Link href="/" passHref>
                  <motion.div className="inline-flex items-center" variants={logoVariants} initial="hidden" animate="visible">
            ...
            {/* Collapse toggle */}
            <div className="p-3" style={{ borderTop: isDark ? '1px solid rgba(255,255,255,0.1)' : '1px solid var(--border-default)' }}>
               <button
                  onClick={() => setCollapsed((prev) => !prev)}
                  className={classNames(
                     'w-full flex items-center justify-center py-2 rounded-md transition-all duration-200 cursor-pointer',
                     isDark ? 'text-white/40 hover:text-white/70 hover:bg-white/5' : 'text-[#0F2552]/30 hover:text-[#0F2552]/60 hover:bg-[#0F2552]/5',
                  )}
            ...
            {/* Tooltip portal (collapsed desktop only) */}
               ...
               createPortal(
                  <div
                     ...
                     style={{
                        ...
                        transform: 'translateY(-50%)', background: '#1a1a2e', color: '#fff',
                        fontSize: '0.75rem', fontWeight: 500, padding: '6px 10px',
                        borderRadius: 6, whiteSpace: 'nowrap', textTransform: 'capitalize',
                        zIndex: 9999, pointerEvents: 'none', boxShadow: '0 4px 12px rgba(0,0,0,0.3)',
                     }}
                  >
                     <span aria-hidden="true" style={{ position: 'absolute', right: '100%', top: '50%', transform: 'translateY(-50%)', border: '5px solid transparent', borderRightColor: '#1a1a2e' }} />
                     {tooltip.label}
                  </div>,
                  document.body,
               )}
         ...
         {/* ── MOBILE sidebar — portalled to <body> so layout containment can't interfere ── */}
         {mounted && createPortal(
            <>
               {/* Backdrop */}
               <div
                  className={classNames(
                     'fixed inset-0 z-[5150] bg-black/40 transition-opacity duration-300',
               ...
               <div
                  className={classNames(
                     'fixed inset-y-0 left-0 z-[5200] flex flex-col overflow-hidden transition-transform duration-300 ease-in-out',
                     isDark ? 'sidebar-dark' : '',
                     mobileOpen ? 'translate-x-0' : '-translate-x-full',
                  )}
                  style={{ width: 260, minWidth: 260, ...sharedStyles }}
               >
                  {/* Logo + close button */}
                  <div className="flex items-center justify-between px-4 py-5">
                     ...
                     <button
                        ...
                        className={classNames(
                           ...
                           isDark ? 'text-white/50 hover:text-white hover:bg-white/10' : 'text-[#0F2552]/40 hover:text-[#0F2552] hover:bg-[#0F2552]/8',
                        )}
